...
const processMessage = (userObj, channelObj, data) => {
  ...
  */
  ...
  if (!msg.indexOf("+") === -1) {
    console.log(
      "Invalid message. Does not mention an amount of Bonusly to give. "
    );
    bot.postMessageToUser(
      userObj.name,
      `Please mention an amount of Bonusly to give.`,
      {}
    );
    return;
  }
  ...
  if (msg.split("+").length > 2) {
    ...
    bot.postMessageToUser(
      userObj.name,
      "Please put only one + sign in your message to specify the amount.",
      {}
    );
    return;
  }
  ...
  const bonuslyReq = [
    {
      ...
      text: {
        type: "mrkdwn",
        text: `You have a new Gambly bet request from ${
          getBonuslyUserFromEmail(getEmailFromSlackUser(userObj)).full_name
  ...
  bot.getUsers().then(e => {
    ...
    if (!receiverObj.name) {
      console.error("Unable to find receiver");
    }
    const userEmail = getEmailFromSlackUser(userObj);
    const userBalance = getBonuslyUserFromEmail(userEmail)["giving_balance"];
    const userFullName = getBonuslyUserFromEmail(userEmail)["full_name"];
    console.log("userBalance: " + userBalance);
    const receiverEmail = getEmailFromSlackUser(receiverObj);
    const receiverBalance = getBonuslyUserFromEmail(receiverEmail)[
      "giving_balance"
    ];
    const receiverFullName = getBonuslyUserFromEmail(receiverEmail)[
      "full_name"
    ];
    console.log("receiverBalance: " + receiverBalance);
    console.log("parseInt(amount): " + parseInt(amount));
    if (userBalance < parseInt(amount)) {
      console.log(userObj.name + " doesn't have enough funds!");
      bot.postMessageToUser(
        userObj.name,
        `${userFullName} has insufficient balance`
      );
      return;
    } else if (receiverBalance < parseInt(amount)) {
      console.log(receiverObj.name + " doesn't have enough funds!");
      bot.postMessageToUser(
        userObj.name,
        `${receiverFullName} has insufficient balance`
      );
      return;
    }
      ...
      atlasWinner(users)
        .then(winner => {
          ...
          bot.postMessageToChannel(
            "general",
            `${bonuslyUser.username} has won the wager!`
          );
        })
        .catch(err => console.log(err));
    }
...
async function atlasWinner(users) {
  ...
  const betId = uuidv1();
  await bets.insertOne({ betId: betId, winner: users[0] });
  await bets.insertOne({ betId: betId, winner: users[1] });

  const winnersList = await bets
    .aggregate([{ $match: { betId: betId } }, { $sample: { size: 1 } }])
    .toArray();
